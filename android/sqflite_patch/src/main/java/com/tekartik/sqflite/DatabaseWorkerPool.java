// Copyright (c) 2019, Alexandre Roux Tekartik. BSD-2-Clause.
// WishLoop: transaction-aware single-worker dispatch; see android/sqflite_patch/PATCHES.md.
        ...
        DatabaseDelegate delegate = database == null ? null : new DatabaseDelegate() {
            ...
            public boolean isInTransaction() {
                return database.getWritableDatabase() != null
                        && database.getWritableDatabase().isOpen() && database.isInTransaction();
            }
        };
...
class SingleDatabaseWorkerPoolImpl implements DatabaseWorkerPool {
    ...
    private final LinkedList<DatabaseTask> pending = new LinkedList<>();
    private DatabaseTask transactionOwner;
    ...
    // A Flutter transaction spans several platform messages. Another connection's
    // BEGIN must not occupy this worker while the owner's COMMIT is still in Dart.
    // Keep the existing single worker, but drain the active connection until its
    // transaction ends (or its database closes) before starting another connection.
    private void drain() {
        while (true) {
            if (transactionOwner != null && !transactionOwner.isInTransaction()) {
                transactionOwner = null;
            }
            DatabaseTask next = null;
            synchronized (pending) {
                ListIterator<DatabaseTask> iterator = pending.listIterator();
                while (iterator.hasNext()) {
                    DatabaseTask candidate = iterator.next();
                    if (transactionOwner == null || Objects.equals(
                            candidate.getDatabaseId(), transactionOwner.getDatabaseId())) {
                        next = candidate;
                        iterator.remove();
                        break;
                    }
            ...
            if (next == null) return;
            next.runnable.run();
            transactionOwner = next.isInTransaction() ? next : null;
        }
    }
}
