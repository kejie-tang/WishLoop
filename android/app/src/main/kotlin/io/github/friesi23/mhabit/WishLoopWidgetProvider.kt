...
class WishLoopWidgetProvider : AppWidgetProvider() {
    companion object {
        ...
        private fun render(context: Context, snapshot: JSONObject?, height: Int): RemoteViews {
            ...
            views.setOnClickPendingIntent(R.id.widget_estimate, launch(context, "wishes"))
            ...
            val estimate = if (day != null) snapshot?.optString("estimate").orEmpty() else ""
            views.setTextViewText(R.id.widget_estimate, estimate)
            views.setViewVisibility(R.id.widget_estimate, if (estimate.isEmpty()) View.GONE else View.VISIBLE)
            ...
            views.setTextColor(R.id.widget_estimate, secondary)
